...
// ─── Search Dropdown ──────────────────────────────────────────────────────────
function UserSearchDropdown({ organizerId, onSelect, excludeIds }: {
  organizerId: number;
  onSelect: (user: UserSearchResult & { _note?: string }) => void;
  ...
  function handleConfirm() {
    ...
    onSelect({ ...selectedUser, _note: note.trim() });
            ...
            {results.map((user) => (
              <button key={user.id} type="button" onClick={() => pickUser(user)}
                ...
                <div className="min-w-0">
                  <p className="font-medium text-white text-sm truncate">{user.f_name} {user.l_name}</p>
  ...
  return (
    <div className="flex items-center gap-3 bg-white/[0.02] p-3 border border-white/5 rounded-xl">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-medium text-white text-sm truncate">{entry.f_name} {entry.l_name}</p>
        <p className="text-gray-500 text-xs truncate">{entry.email}</p>
        {entry.note && <p className="mt-0.5 text-violet-400 text-xs truncate">📝 {entry.note}</p>}
        {entry.ticket_id && (
          <p className="flex items-center gap-1 mt-0.5 text-green-500 text-xs">
            🎫 ticket พิเศษสร้างแล้ว
          </p>
        )}
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <span className="hidden sm:flex items-center gap-1 text-green-400 text-xs">
          <ShieldCheck size={12} /> WL
        </span>
...
export default function WhitelistManager({ organizerId, eventId }: WhitelistManagerProps) {
  ...
  async function handleAdd(user: UserSearchResult & { _note?: string }) {
    ...
    if (!token) return;
    try {
      const result = await addToWhitelist(token, organizerId, eventId, user.id, user._note);
      setList(result.whitelist ?? []);
      flash(`✅ เพิ่ม ${user.f_name} ${user.l_name} เข้า whitelist แล้ว — ticket พิเศษถูกสร้างและส่งไปยัง "ตั๋วของฉัน" แล้ว`);
  ...
  async function handleRemove(wlId: number) {
    ...
    try {
      ...
      setList(updated);
      flash("✅ ลบออกแล้ว — ticket ถูกยกเลิก");
    ...
    <section className="space-y-4">
      <div className="flex justify-between items-center pb-2 border-white/5 border-b">
        <div className="flex items-center gap-2">
          <h2 className="font-semibold text-gray-400 text-sm uppercase tracking-wider">Whitelist</h2>
          {list.length > 0 && (
        ...
        <p className="flex items-center gap-1 text-gray-600 text-xs">
          <ShieldCheck size={12} /> ไม่นับรวมกับยอดขายตั๋ว
        </p>
      </div>
      ...
      <div className="flex gap-2.5 bg-violet-500/5 px-4 py-3 border border-violet-500/15 rounded-xl">
        ...
        <p className="text-gray-400 text-xs leading-relaxed">
          User ใน whitelist จะได้รับ ticket พิเศษอัตโนมัติ
          สามารถดู QR Code ได้ที่หน้า <strong className="text-white">"ตั๋วของฉัน"</strong>
        </p>
      </div>
      ...
      <UserSearchDropdown organizerId={organizerId} onSelect={handleAdd} excludeIds={excludeIds} />
