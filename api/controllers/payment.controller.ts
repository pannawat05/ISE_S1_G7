...
export async function checkout(req: AuthRequest, res: Response) {
  ...
  try {
    const zones = await query<{ id: number; price: number }[]>(
      "SELECT id, price FROM zones WHERE id = ? AND event_id = ? LIMIT 1",
      [zone_id, event_id]
    );
    ...
    if (seat_ids?.length > 0) {
      ...
      if (takenCheck.length > 0) {
        return res.status(409).json({ message: "บางที่นั่งถูกจองแล้ว" });
      }
    }
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      currency: "thb",
      metadata: { userId: String(userId), ticketIds: JSON.stringify(ticketIds) },
    });
...
export async function confirmPayment(req: AuthRequest, res: Response) {
  ...
  try {
    ...
    const pendingTickets = await query<{ ticket_id: number }[]>(
...
export async function getMyTickets(req: AuthRequest, res: Response) {
  ...
  const tickets = await query<{
    id: number; qrcode: string; status: string;
    seat_position: string; zone_name: string; event_name: string;
  }[]>(
    `SELECT 
        t.id, 
        t.qrcode, 
  ...
  const withQR = await Promise.all(
    tickets.map(async (t) => {
      ...
      if (t.status === "paid" || t.status === "checked_in") {
        qr_data_url = await QRCode.toDataURL(JSON.stringify({ id: t.id, code: t.qrcode }));
      }
